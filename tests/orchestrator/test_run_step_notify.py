"""Retired step-count limits must never manufacture escalation notifications."""
from __future__ import annotations

from unittest.mock import MagicMock

from runtime.models import BlockKind, TaskStatus
from runtime.orchestrator import run_step as run_step_mod


def test_legacy_step_count_never_calls_notify_escalated():
    seen: list[dict] = []

    class _FakeOrch:
        def __init__(self):
            self._db = MagicMock()
            self._audit = MagicMock()
            self._settings = MagicMock(max_orchestration_steps=1)
            self._notifier = object()  # truthy

        def notify_escalated(self, **kwargs):
            seen.append(kwargs)

    fake = _FakeOrch()
    task = MagicMock(
        id="TASK-1", status=TaskStatus.PENDING, block_kind=None,
        cancelled_at=None, orchestration_step_count=1,
        parent_task_id=None,  # root — only roots escalate over budget (THR-033 A)
    )
    fake._db.get_task.return_value = task
    # The ordinary atomic claim remains the admission seam.  Losing it makes
    # this focused test stop before executor setup while proving that a legacy
    # count cannot take the former escalation path.
    fake._db.try_claim_for_step.return_value = False
    run_step_mod.run_step_impl(fake, "TASK-1")

    fake._db.try_escalate_runtime.assert_not_called()
    fake._audit.log_escalation.assert_not_called()
    assert seen == []
