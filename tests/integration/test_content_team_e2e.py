...
def test_content_team_pass_path_completes(
    ...
    # All agents use opc report-completion --from-file <json>.
    plan.write_text(
        '#!/usr/bin/env bash\n'
        'set -e\n'
        ...
        '    opc report-completion --org "$org_slug" --from-file "$tmpfile"\n'
        '    rm -f "$tmpfile"\n'
        '\n'
    ...
    # files on disk, so opc recall --fetch-artifact would 404. The artifact
    ...
    # to fake_claude.sh, add an opc recall assertion here.
