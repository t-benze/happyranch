...
def test_agent_compose_from_task_spawns_thread_and_recipient_replies(
    ...
    fake_claude_plan_env.write_text(
        ...
        'cat > "$plan_dir/thread-compose-int.json" << \'ENDJSON\'\n'
        ...
        '--session-id "$SESSION_ID" --from-file "$plan_dir/thread-compose-int.json" >&2\n'
        '\n'
        ...
        '"$TASK_ID" "$SESSION_ID" > "$plan_dir/completion-${TASK_ID}.json"\n'
        '\n'
        'happyranch report-completion --org "$ORG_SLUG" '
        '--from-file "$plan_dir/completion-${TASK_ID}.json" >&2\n'
    )
    ...
    fake_claude_thread_plan_env.write_text(
        ...
        'payload=$(mktemp "$HAPPYRANCH_TEST_PLAN_DIR/thread-reply-XXXXXX.json")\n'
