...
def test_audit_requires_token(tmp_home, app) -> None:
    r = TestClient(app).get("/api/v1/audit")
    assert r.status_code == 401


def test_audit_idle_returns_409(tmp_home, app_idle, auth_headers) -> None:
    r = TestClient(app_idle).get("/api/v1/audit", headers=auth_headers)
    assert r.status_code == 409
    assert r.json()["detail"]["code"] == "no_active_runtime"


def test_audit_returns_all_entries(tmp_home, app, daemon_state, auth_headers) -> None:
    _seed(daemon_state)
    r = TestClient(app).get("/api/v1/audit", headers=auth_headers)
...
def test_audit_filters_by_task_id(tmp_home, app, daemon_state, auth_headers) -> None:
    _seed(daemon_state)
    r = TestClient(app).get(
        "/api/v1/audit", params={"task_id": "TASK-001"}, headers=auth_headers,
    )
    assert r.status_code == 200
    entries = r.json()["entries"]
...
def test_audit_filters_by_action_and_agent(tmp_home, app, daemon_state, auth_headers) -> None:
    _seed(daemon_state)
    r = TestClient(app).get(
        "/api/v1/audit",
...
def test_audit_limit_caps_to_most_recent(tmp_home, app, daemon_state, auth_headers) -> None:
    _seed(daemon_state)
    r = TestClient(app).get(
        "/api/v1/audit", params={"limit": 2}, headers=auth_headers,
    )
    entries = r.json()["entries"]
