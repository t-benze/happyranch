...
def test_backfill_enrollments_imports_known_workspaces(
    ...
    r = TestClient(app).post(
        "/api/v1/orgs/alpha/agents/backfill-enrollments",
        headers=auth_headers,
    )
    assert r.status_code == 200
    body = r.json()
...
def test_backfill_enrollments_is_idempotent(
    ...
    client = TestClient(app)
    r1 = client.post(
        "/api/v1/orgs/alpha/agents/backfill-enrollments", headers=auth_headers,
    )
...
def test_require_eh_auth_talk_path_success(
    ...
    _require_eh_auth(body, org_state)  # no raise
...
def test_require_eh_auth_talk_path_wrong_agent_raises_403(
    ...
    with pytest.raises(HTTPException) as ex:
        _require_eh_auth(body, org_state)
...
def test_require_eh_auth_task_path_success(
    ...
    org_state.sessions.set_active("TASK-100", "engineering_head", "sess-eh")
...
def test_require_eh_auth_task_path_wrong_session_raises_403(
    ...
    org_state.sessions.set_active("TASK-100", "engineering_head", "sess-real")
