...
runtime store (executor_profiles.yaml under daemon home) and audits to
runtime-audit.db; the per-org config.yaml executor_profiles surface is
removed.
"""
...
def _runtime_audit_rows(profile_name: str) -> list:
    """Read runtime-audit.db rows for a profile (THR-107 audit target)."""
    audit_db_path = _daemon_home() / "runtime-audit.db"
    if not audit_db_path.exists():
        return []
    db = Database(audit_db_path)
    try:
        return db.get_audit_logs(f"executor:{profile_name}")
    finally:
        db.close()
...
""")
    ...
    def test_register_records_audit_log(self, app, daemon_state, monkeypatch):
        ...
        # THR-107: audit row goes to the machine-global runtime-audit.db
        # (scope task_id executor:<name>), same as the runtime-level route
        # — NOT to the org's audit log (no org-config write happens).
        rows = _runtime_audit_rows("test-executor")
        assert len(rows) == 1
        row = rows[0]
        ...
        # No org-scoped config audit row is written anymore
        db = daemon_state.orgs["alpha"].db
        assert db.get_audit_logs("config:executor_profiles") == []
...
class TestConcurrentSameToken:
    ...
    def test_concurrent_same_token_exactly_one_succeeds(
        ...
        # Exactly one audit row in runtime-audit.db
        rows = _runtime_audit_rows("test-executor")
        assert len(rows) == 1, (
            f"Expected exactly 1 audit row, got {len(rows)}"
        )
...
class TestStoreWriteFailureDoesNotLeakRegistry:
    ...
    def test_store_write_failure_no_registry_leak(
        ...
        rows = _runtime_audit_rows("test-executor")
        assert len(rows) == 0, (
            f"Expected 0 audit rows, got {len(rows)} — audit should not "
