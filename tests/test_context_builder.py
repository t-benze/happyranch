...
def test_build_settings_json_no_repos(test_settings, tmp_dir, runtime):
    ...
    assert data["permissions"]["allow"] == ["Bash(happyranch:*)"]
    # No repos → no hooks
    assert data["hooks"] == {}
...
def test_ensure_workspace_ready_grants_engineering_head_gh_resolve_rules(
    ...
    assert "Bash(happyranch:*)" in allow
...
def test_ensure_workspace_ready_does_not_grant_gh_to_non_eh_agents(
    ...
    """Only EH resolves PRs/issues; workers stay on the narrow happyranch allowlist."""
    builder = ContextBuilder(test_settings, runtime, slug="test")
    workspace = tmp_dir / "workspaces" / "dev_agent"
    builder.ensure_workspace_ready(
        workspace=workspace,
...
def test_ensure_workspace_ready_does_not_copy_skills_after_cutover(test_settings, tmp_path, runtime):
    ...
    # Cutover: wholesale dump is disabled — no skills should land during bootstrap.
    assert not (workspace / ".claude" / "skills" / "start-task" / "SKILL.md").exists()
    assert not (workspace / ".claude" / "skills" / "make-worktree" / "SKILL.md").exists()
...
def test_generated_claude_md_contains_kb_section(tmp_path):
    ...
    assert "happyranch kb add --agent" in body
    assert "happyranch kb update" in body
