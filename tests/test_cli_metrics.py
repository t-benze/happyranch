...
def test_cmd_metrics_maintenance_confirmation_refusal_prints_hint(capsys) -> None:
    from cli.commands.metrics import cmd_metrics_maintenance

    client = MagicMock()
    client.post.return_value = _response(
        {"detail": {"code": "confirmation_required", "detail": "confirm"}},
        status_code=409,
    )
    ...
    output = capsys.readouterr().out
    assert "--confirm-quiescent" in output
