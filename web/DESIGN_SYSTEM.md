...
| `Composer` | [source:patterns/Composer.tsx#Composer] [story:patterns/Composer.coverage.stories.tsx#Coverage] Patterns / Composer States | ready, abort, error/draft, upload-pending; a submission latches synchronously, disables attach/send/remove while pending, keeps one stable identity per file selection (duplicates stay distinct) and clears draft/chips only while the originating destination is still active; 18px local-extension shell | input/button, `--radius-lg` |
