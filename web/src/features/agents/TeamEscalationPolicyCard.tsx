  ...
  return (
    <PolicyShell>
      ...
      <section aria-labelledby="policy-history-heading" className="border-border-subtle mt-5 border-t pt-4">
        ...
        {!history.isLoading && !history.isError && historyItems.length > 0 && <Button size="sm" variant="ghost" disabled={!history.hasNextPage || history.isFetchingNextPage} aria-label="Load more policy history" onClick={() => void history.fetchNextPage()}>{history.isFetchingNextPage ? 'Loading more history…' : history.hasNextPage ? 'Load more history' : 'End of policy history'}</Button>}
      </section>
      <section aria-labelledby="policy-outcomes-heading" className="border-border-subtle mt-5 border-t pt-4">
        ...
        {outcomes.isLoading ? <p className="text-text-muted mt-2 text-xs">Loading outcomes…</p> : outcomes.isError ? <p role="alert" className="text-tier-red mt-2 text-xs">Could not load outcomes.</p> : !outcomeItems.length ? <p className="text-text-muted mt-2 text-xs">No manager self-evaluation outcomes yet.</p> : <ul className="mt-2 space-y-2">{outcomeItems.map((item) => <li key={item.candidate_id} className="bg-surface-sunken rounded p-2 font-mono text-xs"><div>{item.disposition ?? 'pending'} · {item.disposition_code ?? 'no durable evaluation'}</div><div className="text-text-muted">task {item.root_task_id} · session {item.manager_session_id} · release {item.release_id ?? 'missing'}</div><div className={item.receipt_state === 'complete' ? 'text-tier-green' : 'text-tier-amber'}>{item.receipt_state}</div></li>)}</ul>}
        {!outcomes.isLoading && !outcomes.isError && outcomeItems.length > 0 && <Button size="sm" variant="ghost" disabled={!outcomes.hasNextPage || outcomes.isFetchingNextPage} aria-label="Load more evaluation outcomes" onClick={() => void outcomes.fetchNextPage()}>{outcomes.isFetchingNextPage ? 'Loading more outcomes…' : outcomes.hasNextPage ? 'Load more outcomes' : 'End of evaluation outcomes'}</Button>}
      </section>
    </PolicyShell>
  );
}
