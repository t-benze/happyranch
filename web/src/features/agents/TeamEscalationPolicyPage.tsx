...
export function TeamEscalationPolicyPage(): JSX.Element {
  ...
  if (agents.isLoading || teams.isLoading) return <div className="text-text-muted p-6">Loading agent…</div>;
  ...
  return (
    <div className="bg-surface-canvas h-full overflow-y-auto">
      <section className="mx-auto w-full max-w-4xl p-4 sm:p-6" aria-labelledby="team-policy-page-heading">
        <Button asChild variant="ghost" size="sm"><Link to={routes.detail(policyAgent.name)}>← Back to {displayName(policyAgent.name)}</Link></Button>
