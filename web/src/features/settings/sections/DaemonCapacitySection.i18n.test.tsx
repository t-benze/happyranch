...
describe('DaemonCapacitySection i18n (W2c capacity)', () => {
  test('zh-CN renders localized product copy and keeps every number, key and daemon string verbatim', () => {
    ...
    expect(screen.getByText('当前生效的上限')).toBeInTheDocument();
    expect(screen.getByLabelText(/任务会话上限/)).toHaveValue('3');
    expect(screen.getByLabelText(/受监管会话总上限/)).toHaveValue('10');
    expect(screen.getByLabelText('更改原因')).toHaveAttribute(
      'placeholder',
      '例如：新增第二个团队后队列延迟增加；提高任务槽位。',
    );
    expect(screen.getByRole('button', { name: '保存到下次启动' })).toBeInTheDocument();
    ...
    expect(screen.queryByRole('button', { name: 'Save for next start' })).not.toBeInTheDocument();

    // Verbatim: config keys, daemon guidance, daemon provenance, revision, numbers.
    expect(screen.getByText('queue_workers · host_global_session_cap')).toBeInTheDocument();
    expect(document.body).toHaveTextContent('Starting guidance 4-6.');
    ...
    expect(document.body).toHaveTextContent('任务 3 · 线程 4 · 梦境 1 · 唤醒 1 · 计划 1');
    expect(document.body).toHaveTextContent('值接收于 09:08:07（你的设备时间）');
  });
  ...
  test('a validation error raised in zh-CN re-translates on a locale switch', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '保存到下次启动' }));
    expect(await screen.findByText('任务会话上限为必填项。')).toBeInTheDocument();
    expect(screen.getByText('保存前请检查高亮的字段。未发送任何内容。')).toBeInTheDocument();

    switchTo('en');
    expect(await screen.findByText('Task session limit is required.')).toBeInTheDocument();
    expect(
      screen.getByText('Check the highlighted fields before saving. Nothing was sent.'),
    ).toBeInTheDocument();
    expect(screen.queryByText('任务会话上限为必填项。')).not.toBeInTheDocument();
  });

  test('switching en -> zh-CN -> en keeps the same input node, its value and its focus', async () => {
    const user = userEvent.setup();
    mount('en');
    const input = screen.getByLabelText(/Task session limit/) as HTMLInputElement;
    ...
    const zhInput = screen.getByLabelText(/任务会话上限/);
    ...
    const enInput = screen.getByLabelText(/Task session limit/);
    expect(enInput).toBe(input);
    expect(enInput).toHaveValue('7');
    expect(enInput).toHaveFocus();
  });

  test('zh-CN leave dialog uses its visible title as the one accessible name', async () => {
    const user = userEvent.setup();
