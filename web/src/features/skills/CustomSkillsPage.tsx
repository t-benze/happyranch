...
export function CustomSkillsPage(): JSX.Element {
  ...
  return <div className="h-full overflow-y-auto"><div className="mx-auto w-full max-w-5xl px-4 py-5 md:px-7 md:py-6">
    <header className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><div className="text-fg-subtle text-overline mb-1 tracking-wider uppercase">{removed ? 'Removed' : 'Founder workspace'} · {skills.length}</div><h1 className="text-h2 text-fg">Custom Skills</h1><p className="text-fg-muted text-body-sm mt-1">Guidance visibility only. Eligibility controls who is shown a skill; it never grants permissions or tools.</p></div><Button asChild><Link to={`/orgs/${slug ?? ''}/skills/custom/new`}><Plus />Add custom skill</Link></Button></header>
