...
export function CustomSkillsPage(): JSX.Element {
  ...
  return <div className="h-full overflow-y-auto"><div className="mx-auto w-full max-w-5xl px-4 py-5 md:px-7 md:py-6">
    ...
    {skills.length === 0 ? <EmptyState icon={<Sparkles size={28} />} title="No custom skills yet" body="Create a custom skill to begin managing founder-authored guidance." /> : <ul className="flex flex-col gap-3">{skills.map((skill) => <li key={skill.id ?? skill.skill_id}><CustomSkillCard skill={skill} slug={slug ?? ''} /></li>)}</ul>}
  </div></div>;
}
