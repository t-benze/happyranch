...
/** True when the mutation error carries the seq43 Option A request-identity
 *  code. Reads only the error's public `code` field — compositions reach the
 *  API through the feature hook, never `@/lib/api` directly. */
function isInvalidSlugError(error: unknown): boolean {
  return (
    typeof error === 'object' &&
    error !== null &&
    (error as { code?: unknown }).code === 'invalid_slug'
  );
}

/** Product-language create-error copy. The seq43 Option A request-identity
 *  refusal (HTTP 422 `invalid_slug`) names the literal ASCII grammar; the
 *  permanently-reserved slug keeps its existing receipt wording; everything
 *  else stays the generic fallback. No new field or layout. */
function createErrorMessage(error: unknown): string {
  if (isCustomSkillSlugPermanentlyReserved(error)) {
    return 'This slug is permanently reserved by a removed custom skill. Open the Removed view to inspect its receipt.';
  }
  if (isInvalidSlugError(error)) {
    return 'The slug can use only lower-case ASCII letters a-z, digits 0-9 and single hyphens, and must be 1-64 characters.';
  }
  return 'Could not create this custom skill. Check the details and try again.';
}
