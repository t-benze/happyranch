...
const CUSTOM_FAILED: Detail = {
  ...
  description: 'Quarantined legacy guidance retained only for read-only display.',
...
describe('SkillDetailPage — detail + provenance (THR-092 Slice 2)', () => {
  ...
  test('legacy custom detail has no direct editing or assignment controls', async () => {
    mount(CUSTOM.skill_id);
    await screen.findByText('tourism-partner-playbook');
    expect(screen.queryByRole('link', { name: /edit skill/i })).toBeNull();
    expect(screen.queryByRole('button', { name: /^(assign|unassign) /i })).toBeNull();
  ...
  test('read-only bundled skill exposes provenance but no direct assignment controls', async () => {
    mount(MANAGED.skill_id);
    await screen.findByText('kb-curation');
