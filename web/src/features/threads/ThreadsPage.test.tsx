...
describe('ThreadsPage — list (design-overhaul reshape)', () => {
  ...
  test('lists threads with last speaker (turn budget removed from UI)', async () => {
    sessionStorage.setItem('happyranch.token', 'tok');
    server.use(
      http.get(`/api/v1/orgs/${SLUG}/threads`, () =>
        HttpResponse.json({
    ...
    await waitFor(() => {
      ...
      // Turn budgets should NOT appear (removed per THR-046 msg126)
      expect(screen.queryByText('3/500')).toBeNull();
      expect(screen.queryByText('487/500')).toBeNull();
