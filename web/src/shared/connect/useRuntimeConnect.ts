...
export function buildAdapterConnectPrompt(
  ...
  return [
    ...
    `# validates identity, conformance, path, and dependency facts, creates`,
    `# and binds the "${name}" profile in a single transaction.`,
    `# When you see Connected on this screen the token is consumed.`,
    `# No founder approval wait, no separate bind action needed.`,
    `# Changing the wrapper, dependencies, or capabilities requires`,
    `# re-submission through this same direct-connect path.`,
    ``,
    `# This token is valid for about 30 minutes. This screen updates live.`,
  ].join('\n');
}
