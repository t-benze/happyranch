...
export function buildAdapterConnectPrompt(
  ...
  return [
    ...
    ``,
    `# 1. Create a v1 adapter wrapper executable at exactly the`,
    `#    required_executable_path returned by contract-reference.`,
    `#    Do NOT place it in ~/.happyranch, a project folder, or any`,
    `#    self-chosen path — only the exact canonical path is accepted.`,
    `#    Exact I/O contract:`,
    ...
    `#    body {"executable":"<required_executable_path>","version":"1.0.0",`,
    ...
    `#    The executable field MUST be exactly the required_executable_path`,
    `#    from the contract-reference response — the server rejects any`,
    `#    other location. Dependency records remain absolute path + SHA-256`,
    `#    with normal absolute-path validation (no location constraint).`,
    ``,
