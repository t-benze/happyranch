...
export type { CustomSkill, CustomSkillsView, EligibilityRule } from '@/lib/api/customSkills';
...
export function isCustomSkillSlugPermanentlyReserved(error: unknown): boolean {
  return error instanceof ApiError && error.status === 409 && error.code === 'slug_permanently_reserved';
}
...
export function useCustomSkillsCatalog(enabled = true, view: api.CustomSkillsView = 'current') { const slug = useSlug(); return useQuery({ queryKey: [...key(slug), 'catalog', view], queryFn: () => api.listCustomSkills(slug, view), enabled: enabled && !!slug }); }
