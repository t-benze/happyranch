...
export const FocusAndButtons: Story = { render: () => <div data-pasture-requirement="states"><Section eyebrow="Components" title="Focus, disabled & buttons"><div data-pasture-requirement="buttons" className="grid gap-5"><div data-button-variants className="flex flex-wrap gap-3">{(['default', 'secondary', 'outline', 'ghost', 'destructive', 'link'] as const).map((variant) => <Button key={variant} variant={variant}>{variant}</Button>)}</div><div data-button-sizes className="flex flex-wrap gap-3"><Button size="sm">Small</Button><Button>Default</Button><Button size="lg">Large</Button><Button disabled>Disabled</Button></div><p data-focus-guidance className="text-caption text-text-muted">Use Tab to inspect the token-backed focus-visible ring.</p></div></Section></div> };
