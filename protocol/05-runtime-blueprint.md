...
- **Authority policy operator surface (THR-181 S6b/S7)**: the Engineering Manager Agent page alone receives bounded secret-free immutable history and self-evaluation outcome receipts, with explicit incomplete-linkage state. A release-controlled executable readiness contract replaces the static activation guard; the server still enforces readiness and CAS. Code landing/redeploy/readiness do not themselves create or activate a production policy.
