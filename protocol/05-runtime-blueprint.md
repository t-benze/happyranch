...
- **Pre-escalation authority hook (THR-181 Track A)**: before a current manager-owned Engineering root's proposed escalation is committed, the orchestrator runs exactly one audited LLM authority evaluation of the proposed reason against the immutable, release-controlled policy `engineering/pre-escalation-authority@v1` (`runtime/orchestrator/authority_policy.py`, `runtime/orchestrator/authority.py`, wired at the `run_step_impl` escalate commit point). The policy is semantic authority only; server-owned mechanical fences are non-overridable. The evaluation snapshot carries authoritative server-derived facts with provenance (budget, lineage, active work, adverse child verdicts, partial-work evidence, permission/schema digests) and a server-PROVEN must-escalate fact forces ESCALATE regardless of reason prose; candidate identity is bound to the immutable `task_results` row id, and the final continuation CAS atomically re-validates the full fence set at consumption time. Semantic results are `ESCALATE` (existing path) and `CONTINUE_SAME_ROOT` (named clause + exact permitted action; same-root return to pending + re-enqueue). Everything else fails closed to ESCALATE. Full contract: [05c-orchestrator.md](05c-orchestrator.md) §THR-181.
