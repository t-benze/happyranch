...
**Immutable release-controlled policy authority.** Before a **current manager-owned Engineering root's** proposed escalation (`decision.action == "escalate"`) is committed, the orchestrator runs exactly **one audited LLM authority evaluation** of the proposed reason against the immutable, release-controlled Engineering policy `engineering/pre-escalation-authority@v1` (`runtime/orchestrator/authority_policy.py`). The policy is code-and-deploy controlled under the accepted shared-identity posture: there is no mutable database/config activation switch, and agents/managers cannot self-modify or self-activate their governing policy. Historical census eligibility is NOT a prerequisite and is NEVER consulted — reachability depends only on a release-controlled policy existing for the manager's team and the root being current/manager-owned.
...
**Audit denominator.** Every hook-eligible attempt records exactly one `authority_hook` audit_log outcome row (`continued_same_root` | `escalated` | `ineligible` | `cas_lost` | `cancelled_stale` | `evaluator_failure` | `audit_failure` | `capture_failure`), reconciled to a terminal recorded outcome; a continuation is impossible unless that row says `continued_same_root`. Each candidate carries immutable input provenance (root/session/manager/team identity, causal event id+digest, reason digest, policy/prompt/model ids+versions+digests, snapshot digest, fence results, structured server facts) plus the single evaluation (disposition, code, response digest) and the four audit events. Raw credentials/prose/model exchanges are never persisted.

**Restart/CAS behavior.** Candidate identity is derived from the **immutable task-result/session causality** — the persisted `task_results` row id that produced the escalate decision (`result:<row id>`) — never from a freshly written orchestration-step audit id, so real restart/recovery re-entry (`_consume_completion_report` via the boot sweep or zombie reaper, which rebuild the report from the SAME row) maps to the SAME candidate: no second candidate and no second evaluation are possible. The DB is the single-evaluation and exactly-once-consumption arbiter (UNIQUE candidate id on `authority_evaluations`; `created -> evaluated -> consumed` CAS; append-only triggers; lifecycle guard trigger). A restart left in any state — claimed/created, evaluation-missing, evaluated-but-unconsumed, already-consumed — fails closed: a replayed attempt loses the claim (`cas_lost`, `ineligible`, or `cancelled_stale`), never re-evaluates, never continues. A `continue_same_root` commit is atomic with its audit rows (`commit_authority_continue_same_root`) AND **atomically re-validates the complete current fence set at consumption time** — candidate/policy/input identity, manager ownership and session, exact team, root status, cancellation, block/active-work, revisit/thread/successor lineage, orchestration and revise budgets (against the current ceilings), zombie/partial-work evidence, and adverse child verdicts — inside the same transaction as the continuation + audit rows, so a fence that changed while the evaluator ran can never slip through a stale verdict; post-commit re-enqueue is best-effort and the run-step claim CAS remains at-most-once.

**Single-use continuation envelope (founder option B, Unit 1 — implemented).** Every `CONTINUE_SAME_ROOT` commit atomically mints a single-use `authority_continue_envelopes` row (`active`), bound 1:1 to the evaluation/candidate, the immutable causal `task_results` row, the matched policy clause, and the exact permitted action, plus manager/team/root/session and policy id/version/digest. DB-level enforcement (no-delete, identity-immutable, and `active -> consumed | violated` lifecycle triggers) means a spent envelope can never re-activate or be rewritten. While the envelope is ACTIVE the daemon-mediated completion/decision acceptance point (`_consume_completion_report`, shared by run-step inline consumption, the daemon boot sweep, and the zombie reaper) accepts ONLY the exact permitted decision — `done` (routine completion of the SAME root): the envelope is consumed exactly once (`active -> consumed`) atomically with its audit row, and the ordinary task-status CAS remains the at-most-once arbiter. EVERY other decision/status family from the continued turn — escalate (the authority hook is NOT re-run: the continuation is single-use and cannot re-grant), supersede/successor/fresh-root/revisit, child/delegate/fanout, blocked, cross-root/team action, external commitment, protected-boundary mutation, malformed/ambiguous output, stale/replayed/nested/callback/recovery attempt — spends the envelope as `violated` (exactly-once CAS + atomic audit row) and fails closed into the EXISTING ordinary founder-escalation path (try_escalate CAS → `escalation` audit row → founder notification → thread projection) with a server-derived reason; the attempt is audited, never silently discarded. Cancellation during the continued turn, session failure, and any `_fail`/terminal path spend an active envelope fail-closed. A replay of the ORIGINAL escalate's immutable row is NOT the continued turn — it rides the ordinary fail-closed path (hook eligibility/claim CAS). The continued-turn prompt header is shown ONLY while an active envelope exists (no envelope → no continuation header → ordinary turn). The audit denominator records issuance (`authority_continued_same_root` names the envelope id), consumption (`authority_continue_envelope_consumed`), and violation (`authority_continue_envelope_violated`) with bounded non-secret-bearing fields (decision family + error code, never raw prose).

**Executor turn-scoped allow set (founder option B, Unit 2 — implemented).** The continued manager turn's EXECUTOR process is mechanically narrowed, derived from the ACTIVE envelope identity immediately before launch (`resolve_continuation_turn_allow_set` — the launch-time envelope identity recheck; the resolver returns None for an ordinary no-envelope turn and a `refused` marker on identity mismatch or an unsupported permitted action). The continued turn may use ONLY the minimum commands/tools needed to perform the single permitted same-root action (`done`) and report it — it never inherits the ordinary manager executor permissions, and global/baseline permission generation is never touched. Claude launches with `--allowedTools` reduced to `Bash(happyranch report-completion *)` plus the Read/Grep/Glob/Write tools needed to author the completion payload (no general bash, no git/gh, no Edit, and no other `happyranch` verb — jobs/threads/kb/memory/artifacts/cancel/revisit/etc. are unreachable from the narrowed turn); opencode runs under a turn-scoped `opencode.json` permission map narrowed the same way (daemon-owned writer; the ordinary per-agent map is restored after the session, fail-closed on error). Executors WITHOUT a runtime-owned per-command allow surface — codex sandbox flags, pi, generic CLI, custom adapters — CANNOT be mechanically narrowed to `happyranch report-completion`-only, so the continued turn is REFUSED pre-launch: the envelope is spent `violated` (bounded audit row: `decision_family=launch_refused` + error code, never raw prose) and the root enters the EXISTING ordinary founder-escalation path — an unrestricted continued turn is never launched on them. The daemon-side decision gate (Unit 1) remains the authoritative acceptance fence; the executor allow set is defense in depth, never a replacement. No fixed phrase, closed pattern, or reason truthfulness is safety proof on its own — the mechanical envelope is the fence.

**Unit-2 fail-closed lifecycle clarification (TASK-6101).** Durable envelope-history absence alone identifies an ordinary causal turn. ACTIVE history derives the narrowed allow set; consumed, violated, stale, malformed, mismatched, unsupported, or uncertain history refuses into the existing founder-escalation lifecycle. The full immutable envelope/root/session/manager/team/causal-result/policy/clause/action identity is re-read directly adjacent to every backend/Popen attempt, including after admission and before every throttle retry; drift or database uncertainty causes zero launch. This is the authoritative interpretation of “immediately before launch” above.
