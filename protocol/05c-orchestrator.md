...
Telemetry: each successful `run_step` claim increments the persisted,
monotonic `orchestration_step_count` on the task. It is not an admission or
continuation budget. The legacy `max_orchestration_steps` configuration remains
accepted solely for compatibility and is inert. The independent
`max_revise_rounds` budget (org_config, 0 = disabled) still caps genuine revise
cycles (worker-of-record re-delegations). Each revise increments
`revision_count`; when `revision_count >= max_revise_rounds` the next genuine
revise performs its existing deliberate stop-with-best: non-root fails back to
parent and root escalates. The stop is explicitly NOT auto-revisited.
