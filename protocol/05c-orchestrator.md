...
**Intermediate state (honest).** Unit 2 — the executor turn-scoped allow-set narrowing (claude `--allowedTools` reduced to the baseline `happyranch` channel, executors without a runtime-owned per-command allow surface) and the alternate daemon-route rejection (jobs/threads) — is NOT yet implemented. Until Unit 2 lands, the continued turn's executor process is the ordinary executor; the mechanically restricted surface is the daemon-side decision acceptance above. No fixed phrase, closed pattern, or reason truthfulness is safety proof on its own — the mechanical envelope is the fence.
