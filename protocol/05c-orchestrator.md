...
changed-artifact retry. That specific unchanged retry is a nonterminal conflict:
only its temporary reservation is released, preserving the unexpired parent and
first-candidate evidence for a genuinely changed candidate. This is not `/retry` and the
