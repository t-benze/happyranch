...
**Single-use continuation envelope (founder option B, Unit 1 — implemented).** Every CONTINUE_SAME_ROOT commit atomically mints a single-use `authority_continue_envelopes` row bound to the evaluation/candidate, the immutable causal `task_results` row, the matched policy clause, the exact permitted action, manager/team/root/session, and policy id/version/digest (`active -> consumed | violated` exactly-once; no-delete + identity-immutable + lifecycle-guard triggers). While the envelope is ACTIVE the daemon-mediated decision acceptance point (`_consume_completion_report` — run-step inline, boot sweep, zombie reaper) accepts ONLY the exact permitted decision `done` (routine completion of the SAME root); every other decision/status family from the continued turn — escalate (hook NOT re-run), supersede/fresh-root/revisit, delegate/fanout, blocked, cross-root/team action, external commitment, protected-boundary mutation, malformed/ambiguous or stale/replayed/nested/callback/recovery output — is audited (bounded, no raw prose) and fails closed into the EXISTING ordinary founder-escalation path, never silently discarded. Cancellation/session-failure/terminal paths spend an active envelope fail-closed; a replay of the original escalate row rides the ordinary path (never the envelope gate); the continued-turn prompt header appears only while an envelope is ACTIVE. Unit 2 (implemented): the continued turn's EXECUTOR launch is also mechanically narrowed — the launch resolver derives a turn-scoped allow set from the ACTIVE envelope identity immediately before launch; claude's `--allowedTools` is reduced to the single `happyranch report-completion` channel (+ the Read/Grep/Glob/Write tools needed to author the report), opencode runs under a turn-scoped permission map narrowed the same way (restored afterwards), and executors without a runtime-owned per-command allow surface (codex/pi/generic CLI/custom adapters) refuse the continued turn pre-launch into the ordinary founder-escalation lifecycle — an unrestricted continued turn is never launched; baseline/global permission generation is never touched.

For Unit 2, durable envelope-history absence alone is ordinary. ACTIVE history derives the narrowed allow set; any non-ACTIVE, malformed, mismatched, unsupported, stale, or uncertain history refuses through the existing founder-escalation lifecycle. The complete immutable identity is revalidated adjacent to every actual backend/Popen attempt, including admission-delayed and retry attempts; drift or read uncertainty causes zero launch.
