...
- `"supersede"` — THR-152 phase-1 internal pilot only. Its entire payload is exactly `{action, successor_brief, rationale}`; both strings are nonblank. It is default OFF and enabled only by the operator kill-switch environment gate for one configured pilot team. The target, manager, session, and team are derived from the current claimed root server-side. It rejects all mixed/override fields and fails closed for every Founder-gated concern. It creates one pending same-manager/same-team successor and terminalizes the predecessor as `superseded` atomically, preserving literal briefs and SHA-256 hashes in append-only bidirectional provenance/audit rows. It is not a Founder escalation or approval, emits no Founder notification, and has no thread-origin behavior until phase 2.
