...
One process-local org publication fence is acquired before the workspace lock and any DB transaction. Every task/thread/wake/dream/schedule/bootstrap producer enters it through `materialize_workspace_skills`; executor switching enters through `materialize_workspace_skills_union`. A materializer holds it from resolver selection through canonical generation and both `.claude/skills` and `.agents/skills` repairs. Purge holds the same fence through `BEGIN IMMEDIATE` commit, which is the linearization point. Process death releases the fence and the SQLite tombstone remains authoritative. This does not alter host capacity/admission ordering, introduce a distributed lease, or revoke a running session.
