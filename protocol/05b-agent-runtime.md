...
Canonical custom-skill construction re-reads the authoritative skill row after candidate selection and package build, immediately before publishing each canonical spec. If purge committed during that window, the spec is excluded from the returned set and therefore cannot be newly linked by either provider-root repair; a concurrent current-version change also fails closed. Purge itself is a synchronous `BEGIN IMMEDIATE` transaction. There is deliberately no org-wide generation or launch fence, no callback/token plumbing in producers, supervisors, or executors, and no running-session revocation or wait. Existing canonical bytes and historical links remain retained; later materialization withdraws links through the ordinary reconciliation path.
