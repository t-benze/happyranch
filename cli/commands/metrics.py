...
def cmd_metrics_maintenance(args: argparse.Namespace) -> None:
    ...
    print(f"  vacuum:            {body.get('vacuum', 'ok')}")
    ...
    print(f"  snapshot bytes:    {before['total_snapshot_bytes']} -> {after['total_snapshot_bytes']}")
    print(f"  route-label count: {before['route_label_count']} -> {after['route_label_count']}")
