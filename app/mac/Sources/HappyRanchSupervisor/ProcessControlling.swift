...
///
/// The `terminationHandler` parameter is wired BEFORE the underlying process
/// runs, making handler registration and launch atomic — no exit callback can
/// be dropped by a fast-exiting child.
public protocol ProcessControlling: AnyObject, Sendable {
    ...
    ///
    /// - Parameter terminationHandler: Wired BEFORE the process runs so no
    ///   exit callback is dropped.  Delivers the EXITED handle (per-launch
    ///   snapshot), NOT the controller.
    @discardableResult
    func launch(
        executableURL: URL,
        arguments: [String],
