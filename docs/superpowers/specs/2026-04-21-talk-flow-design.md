...
`learnings[].text` is the full learning text; the daemon appends each entry to the agent's `learnings.md` in the same format as `opc learning` (one bullet per line, no per-line provenance — the existing file format is unchanged). Talk-originated learnings bypass the session/task-id matching the `/agents/{name}/learnings` HTTP route enforces for task callbacks; the daemon's `/talks/{id}/end` endpoint calls the same internal append helper directly. Provenance lives in the `talks` row (`new_learnings_count`) and the `talk_ended` audit entry, not in `learnings.md` itself. `kb_slugs` is a list of slugs that the skill ALREADY wrote via `opc kb add` earlier in its body — `/talk end` only records which ones came from this talk, it does NOT re-write them.
