...
| `2026-08-25-thread-rename-and-pinning-design.md` | implemented | Founder-only thread rename + pin (THR-209 Phase 1). Current contracts: `docs/agent-guides/features-and-invariants.md` (Threads), routes in `runtime/daemon/routes/threads.py`, pin storage `threads.pinned_at`, web UI in `web/src/features/threads/ThreadsPage.tsx`. |
| `2026-08-26-managed-remote-access-contract.md` | current | Normative managed remote-access contract (THR-097 merge unit A, TASK-5771): one Headscale cell per customer, ciphertext-only DERP, supervised portable Python connector, loopback daemon + final-hop bearer injection, explicit allow-list, credential/failure taxonomy, hostile threat fixtures. Machine-readable contract: `tests/contract/managed_remote_access/`; no runtime implementation yet (units B–D outside this PR). |
