...
**GH-688 Phase 1 Slice C presentation (TASK-5553 purpose fidelity; TASK-5589 compact rail; TASK-6200 held correction).** The web UI surfaces the authoritative pair state from the `reply_delivery` wire list — never from inferred per-message rows. Producer precedence is `running > queued > held > retry_required > settled omission`; `held` requires both an OPEN exchange and matching HELD participant deferral. The rail prioritizes individually distinguishable `running` pairs, groups static `queued` pairs behind an accessible disclosure, renders `held` as healthy neutral waiting (rail-only, never typing/subprocess/fault), and keeps genuine `retry_required` visible as a diagnostic with a reliable current terminal reason where available. Released/suppressed/closed/missing-half deferrals never mask retry; stale terminal reason never proves classification. Fully settled pairs are absent. **Purpose fidelity:** every `responder_status` wire entry carries the authoritative invocation `purpose` (`reply` | `task_followup`; BOOTSTRAP stays excluded from the grouped query); in-flight classification/dedup on the web uses that wire purpose, never the triggering row's kind — a coalesced REPLY range anchored on a system row is correctly suppressed next to its pair row (exactly one replying bubble) while a same-agent TASK_FOLLOWUP stays visible. The six reply-wake lifecycle audit actions remain unchanged.
