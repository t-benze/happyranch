...
The supervisor runs these before binding and continuously: readiness loss stops the listener immediately; the systemd `Type=notify` contract reports `READY=1`/`WATCHDOG=1`/`STOPPING=1` via sd_notify. **`READY=1` is emitted ONLY after the provider actually started — the listener proven bound — never after a bind/start failure** (a failed start reports `STATUS=provider failed to start; no listener` and the loop retries; the service is never reported ready without a listener). **A RUNNABLE configuration REQUIRES a concrete provider/listener: a provider-less `run` configuration is rejected at startup** (`ConnectorConfigError`, CLI exit 1) and never enters the notify loop, so `READY=1` can never be emitted without a listener. Non-run construction (status/readiness/diagnose/install) remains valid without a provider.
