...
Phase unit 3A (TASK-6032) ships the **production customer-owned-network (Supported-DIY) provider adapter** and the bounded DIY product path: explicit local pairing and re-pairing ceremony, customer-network configuration with safe validation, secret-free diagnostics, secure credential storage/removal through the already-approved abstractions, connector-enforced binding/allow-list, revocation/lost-device/recovery flows, and truthful user-visible lifecycle/error states. It terminates at the **same connector contract and allow-list** as every other lane (§12) and preserves every fixed invariant. **The existing `lab_provider` remains LAB-ONLY** — it is not renamed, relabeled, or replaced, and its synthetic tests do not close THR-034.
...
`ConnectorConfig.diy.network` selects the customer-owned network: `mode: "tailscale"` (ride-installed system Tailscale — `tailscale ip -4` resolves the customer's own tailnet address; no embedded tsnet, no new dependency) or `mode: "explicit"` with a concrete address. Bind-address validation refuses wildcard, loopback, multicast, link-local, reserved/broadcast, and non-IPv4 addresses (fail closed — a connector listens only on a concrete address on the customer's own network). The adapter requires the explicit `--diy` opt-in on `run` (parallel to `--lab-only`); lab and diy providers are mutually exclusive; a provider-less run is rejected (never `READY=1`).
...
`tests/remote_access/test_diy_acceptance.py` (`-m integration`; skipped with an explicit reason on hosts without a non-loopback IPv4) runs a REAL away-client process (the THR-034 wire contract) over a REAL network path to the REAL shipping connector (`python -m runtime.remote_access.cli run --diy`) forwarding to a REAL loopback daemon. Proven: allowed route success (daemon receives the injected bearer on the final hop), forbidden route denial, direct remote daemon attempt refused (daemon loopback-only), daemon bearer never usable as a pairing credential, replayed code denial, re-pair invalidating old authority, revocation persisted across connector restart, removed-credential denial, network/control-plane outage fail-closed (daemon down → listener stops → client refused; daemon back → supervised listener recovers), and credential/token leakage scans (credentials only in the intended client redeem output; bearer never in connector logs; digests only in trust state).

**Honest residual gaps (reported, not fabricated):** this implementation/acceptance ran on a Linux host with no macOS binary and no Tailscale/headscale client, so the genuine signed-macOS-client launch/Keychain surface and the WireGuard/tailnet transport hop remain unproven here — the inherited THR-034 signed-device acceptance is not closed by this unit. The connector serves the identical wire contract the macOS `ClientBridge` speaks, and the client-side ceremony/UX wiring in the macOS app remains a reported follow-up. Managed persistence schema/auth/enrollment, production Headscale/DERP provisioning, tsnet defaults, telemetry, pilot/pricing/launch, and DIY-completes-hosting claims remain out of scope.
