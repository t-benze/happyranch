...
The supervisor runs these before binding and continuously: readiness loss stops the listener immediately; the systemd `Type=notify` contract reports `READY=1`/`WATCHDOG=1`/`STOPPING=1` via sd_notify. **`READY=1` is emitted ONLY after the provider actually started — the listener proven bound — never after a bind/start failure** (a failed start reports `STATUS=provider failed to start; no listener` and the loop retries; the service is never reported ready without a listener).
