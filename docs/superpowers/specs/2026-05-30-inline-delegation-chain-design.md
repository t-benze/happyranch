...
| Duplicate/late terminal delivery of an already-advanced leg | At-most-once terminal consumption. A terminal report is consumed only while its child is the chain's actual in-flight leg. The chain advances one leg at a time, so the in-flight leg is the ONLY child that is not yet terminal; if any OTHER sibling is still non-terminal, the completed child's leg has already been superseded by a later-spawned leg and the duplicate/late delivery is a no-op — no wake, no `active_chain` clear, no re-interpretation against the later leg's expectation (which would otherwise reach `verdict_mismatch` and clear the chain, letting the later leg bypass its gate). Enforced in `_enqueue_parent_if_waiting`'s COMPLETED branch before `_advance_chain_for_completed_child`. |
...
- **At-most-once terminal consumption.** A terminal report is consumed only while its child is the chain's actual in-flight leg (the only non-terminal sibling). A stale duplicate/late delivery of an already-advanced leg is a no-op — never a wake, `active_chain` clear, or re-interpretation against a later leg's expectation. Enforced in `_enqueue_parent_if_waiting`'s COMPLETED branch before `_advance_chain_for_completed_child`.
