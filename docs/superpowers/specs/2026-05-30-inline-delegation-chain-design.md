...
| `code_reviewer` leg with a downstream leg omits `expect_verdict` | Fail-closed. Rejected at declaration (validation) AND, for any persisted in-flight chain, the leg wakes/clears rather than advancing — the orchestrator cannot distinguish `APPROVE` (advance) from `REQUEST_CHANGES` (wake) without a gate, so an omitted-gate reviewer never auto-dispatches QA. This covers BOTH the first leg and later legs: the chain payload persists only `first_leg_expect_verdict`, not the first leg's agent, so the execution seam derives the completed leg's identity from the completed child's `assigned_agent` (DB truth) to fail-closed a FIRST-leg reviewer too. A `code_reviewer` FINAL leg (no downstream) is unaffected (wakes chain-complete). Ordinary non-review legs may omit `expect_verdict` and advance unconditionally. |
| Duplicate/late terminal delivery of an already-advanced leg | At-most-once terminal consumption keyed on the durable ownership marker. `active_chain` persists `in_flight_child_id` — the EXACT id of the child currently in flight — written atomically at materialization (initial delegate + fanout carrier) and rotated atomically on every auto-advance. A terminal report is consumed only while its child's id EXACTLY matches `in_flight_child_id`; it is never inferred from sibling terminal state, agent equality, ordering, or the latest report (those all fail in a valid replay ordering where the next leg has already written its terminal but not yet been delivered). A stale/late duplicate is a no-op in `_enqueue_parent_if_waiting` BEFORE anything can clear state, wake, advance, log audit, fail/complete a carrier, or mutate a fanout barrier. |
| Legacy `active_chain` payload without `in_flight_child_id` | Fail-closed compatibility. The only unambiguous bridge is stale detection — if a LATER leg is still in flight (some OTHER sibling non-terminal), the completed child is definitively superseded and the delivery is a no-op. Otherwise ownership is ambiguous → clear the chain and wake the parent with NO downstream spawn; a carrier FAILS rather than falsely completing. |
...
- **At-most-once terminal consumption, keyed on the durable `in_flight_child_id` marker.** A terminal report is consumed only while its child's id EXACTLY matches the chain's current in-flight child (persisted on `active_chain` and rotated atomically on every materialization/advance). A stale/late duplicate is a no-op — never a wake, `active_chain` clear, or re-interpretation against a later leg's expectation. Enforced in `_enqueue_parent_if_waiting` before `_advance_chain_for_completed_child`.
- **`in_flight_child_id` is written atomically with the child it names.** Every chain materialization (initial delegate + fanout carrier) and every auto-advance sets `in_flight_child_id` to the exact spawned child id inside the same transaction that inserts the child. A crash or write failure rolls back both — no chain state pointing at a child that never materialized.
