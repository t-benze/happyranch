...
>
> **THR-032 Phase 2 (PUSH memory digest — mechanism A).** The `MemoryStore.build_memory_digest()` method produces a salience-ranked, pointer-first, char-budgeted digest; `Orchestrator._build_agent_prompt` injects it as a `=== MEMORY-DIGEST (system) ===` block into every agent spawn prompt (harness-agnostic). In-scope directive entries (provenance=directive, scope matches digest scope) render as full body before pointer lines; all other entries are pointer-only. Directives that don't fit the budget fall back to pointer lines (THR-091 WS-B seq7 founder ratification). The `memory_digest_budget` field (default 1500, org-configurable) controls the budget; 0 disables the digest. Founder-ratified at THR-032 seq 48.
>
> **THR-032 P3a (explicit lifecycle transition API).** The `MemoryStore.set_lifecycle()` method supports manual lifecycle transitions (valid → superseded, valid → evicted, superseded → evicted, evicted → valid, superseded → valid) with audit-only `reason` (never written to frontmatter), promoted-lock enforcement, permanent `LRN-`→`MEM-` id resolution, and no hard delete. The `PATCH /memory/entries/{id}/lifecycle` route and `happyranch memory lifecycle` CLI command enable explicit eviction, supersession, and restoration. The forward-only audit row `memory_lifecycle_changed` records `id`, `from_lifecycle`, `to_lifecycle`, `reason`, and `source` (always `"manual"` in P3a). No compaction sweeps, search-ranking changes, or KB federation are included in P3a.
>
> **THR-032 P3b (manual memory compaction).** `MemoryStore.compact(dry_run=True/False)` performs manual soft-eviction with conservative protections. Eligible candidates: stale low-salience entries and superseded entries beyond grace period. Protected: promoted, directive, team/org-scoped, cross-referenced, and already-evicted items. Dry-run writes nothing; apply recomputes under serialized write protection. When a new entry `supersedes: MEM-NNN`, the target is auto-marked `superseded`. Route `POST /memory/entries/compact` and CLI `happyranch memory compact --dry-run|--apply`. No hard deletes. No automatic scheduling.
>
...
>
> **Session-correlation design (founder-approved seq32):** The `--session-id` flag is passed by the agent CLI as an optional query parameter on `memory get` and `memory search` requests. Before any correlation metadata is attached, the daemon **validates** the session via the server-side `SessionTracker.get_context_by_session()` (four-part provenance: org_slug, task_id, agent_name, session_id). Only a server-side-verified context whose agent matches the route agent qualifies for same-session attribution. Unvalidated, stale, cleared, unknown, or mismatched-agent sessions receive `source=explicit_or_other` with no `task_id`/`session_id` correlation. The validation prevents cross-task cross-credit: a later task submitting an earlier session ID cannot receive `source=digest` for a different task's impression. The validated `task_id` is stored in the `memory_read` and `memory_search` payloads and used by `_resolve_read_source` to filter impressions/searches by task_id. **Browser/TS mirror:** The optional `session_id` query parameter is reflected in `web/src/lib/api/agents.ts` (`getMemory` and `searchMemory` accept optional `sessionId?: string`). The OpenAPI snapshot (`tests/contract/openapi.json`) includes the parameter on both routes.
>
> **Operator CLI report (`happyranch memory report`):** Uses the existing authenticated audit read surface with **cursor pagination** (not fixed first-page caps). Agent roles are fetched from the authoritative `/agents` read surface (not a caller-supplied `--role-map` JSON file). Computes from `memory_digest_impression`, `memory_read`, and `memory_search` rows. Outputs JSON and human-readable form. **Per-session denominators**: digest pull-through uses per-session unique shown/read counts (not globally unioned across sessions). Only correlated read rows (with both `session_id` and `task_id`) are included; legacy/untrusted/unmatched rows are excluded rather than treated as misses. Pre-registers:
...
>
> **Hard boundaries preserved:** No SQLite schema migration/column or task_id-semantic change. No authentication/bearer-token/permission-model/Feishu/notification change. No dependency addition. No memory frontmatter write at read time. No digest-budget/pointer/full-body behavior change. No raw search query/body logging. No unrelated refactoring.
>
> **Founder authorization (THR-091 seq32, TASK-4014):** The optional `session_id` query parameter on `memory get` and `memory search` routes is founder-approved. The OpenAPI snapshot and TS/browser client mirror (`web/src/lib/api/agents.ts`) include this parameter.
>
> **Tuple-verified correlation (TASK-4014 fix-forward):** Report computation builds a validated `(agent, task_id, session_id)` tuple map from trusted impressions. Each read row is verified against the impression's agent, task_id, and session_id before being included in pull-through/search-absent denominators. Rows with mismatched agent or task_id are excluded as untrusted. This prevents a malformed/legacy mismatched row from affecting pull-through or search denominators.
>
