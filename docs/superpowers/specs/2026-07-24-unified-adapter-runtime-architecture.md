...
| **Submit & Connect (scoped, THR-107 seq363)** | **Normal Connect path.** Settings/onboarding mints an adapter-purpose, intended-profile scoped, loopback-only, short-lived/single-use ``hrreg_`` token. The candidate CLI writes the conformance-passing adapter wrapper at the exact canonical ``required_executable_path`` returned by contract-reference, then submits via ``POST /runtime/adapters/submit``. The server atomically creates, approves, and binds the intended custom profile in a single transaction — the normal result is Connected (``eligibility: already_bound``) with no PENDING wait, approve action, or separate bind action. Idempotent only for the same durable snapshot/profile binding; incompatible replays/conflicts fail closed. | Registration-token-scoped, loopback-only, adapter-purpose, intended-profile gated. Token is single-use (consumed on success, released on failure). |
| **Approve (master-bearer/legacy)** | **Founder-gated.** For the master-bearer ``/register`` path only: the founder must explicitly approve the adapter registration via POST /api/v1/runtime/adapters/{adapter_id}/approve (D4). The approval request binds the exact durable artifact snapshot. Any mismatch, missing adapter, non-pending state, or already-approved incompatible repeat fails before persistence. Exact-idempotence: same stored facts + same approval state → no-op. **THR-107 seq237**: For adapters with a nonempty ``intended_profile_name``, approval atomically approves the snapshot AND creates/binds that same named custom profile in one transaction. | The lock serializes atomic writes. No token consumption (master bearer). |
...
| **D14** | Approve scoped-submission direct register-and-connect (seq363)? | Brief TASK-4498 | Replaces the scoped submission's PENDING-only lifecycle with a production-seam direct register-and-connect transaction. Server atomically creates, approves, and binds the intended custom profile in one step — no PENDING wait, no separate founder approval, no client-side bind. Master-bearer /register remains a legacy/operator path. | **IMPLEMENTED (TASK-4498, founder seq363, August 2026).** |
