...
## PR837 B2a current-contract preservation

The three B2a paragraphs introduced by PR837 (`cba8705d`) in
`protocol/05b-agent-runtime.md` (executor-boundary and scratch sections) and
`protocol/05c-orchestrator.md` (scratch observation section) move verbatim to
`docs/agent-guides/features-and-invariants.md`, under
“Dormant B2a task-scratch observation”. This preserves complete durable
evidence, shared-deadline admission, non-preemptible started reads, measured
zero versus unavailable, action-time B2b recollection and B3 coverage authority.
It adds no collector caller, permit, activation or deployment.
