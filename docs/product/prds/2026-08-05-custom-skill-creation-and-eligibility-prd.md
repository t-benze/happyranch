...
3. The service validates the exact candidate version before any persistence. A separate request-identity gate first admits the logical new-write slug only when it is a literal ASCII `^[a-z0-9]+(?:-[a-z0-9]+)*$` value of 1-64 characters with no leading/trailing/consecutive hyphen (full-string match, no Unicode normalization, case folding or transliteration; the same predicate governs the frontmatter `name`). A non-conforming logical identity — non-ASCII, lookalike, fullwidth, non-ASCII digit, uppercase ASCII, over-length or hyphen-boundary — is refused BEFORE validation, dry materialization, artifact-key construction and any durable write with HTTP 422 `invalid_slug` and zero residue; this ASCII-only logical slug is an explicit HappyRanch admission restriction (the Agent Skills standard and its `skills-ref` support Unicode names). Validation itself then checks the supported SKILL.md authoring contract (THR-262 / seq27: a column-zero `---` frontmatter fence, a YAML mapping, a closing `---` fence; the body-heading requirement is retired and the body may be empty) with a closed top-level allowlist of `name`, `description`, `license`, `compatibility` and `metadata` — every other top-level key is admission-policy invalid by presence, whatever its value — plus a required ASCII `name` equal to the logical slug, a required non-empty `description`, optional field typing, package structure, protected slug/path collision, supported reference/asset limits, and prohibited executable/permission-bearing content. A pass is technical evidence, not a claim that the guidance is good.
4. A valid version is saved and becomes the current editable custom-skill version; the catalog `description` is projected from the validated frontmatter `description` in the same transaction that advances the current pointer. An INVALID candidate is also saved — as immutable validation/provenance evidence with its deterministic findings, author provenance, and content-addressed artifact — but it never displaces an existing valid current version (THR-210 PR 1), never projects a description, and is never eligible or materializable. Initial creation with no prior version sets the first version as the current pointer regardless of validity (a NULL pointer is unreadable by list/detail consumers and uneditable), so a skill whose first version is invalid is visible as `current_version_invalid` until a valid successor advances the pointer. Pre-THR-262 records — including heading-first versions stored valid and PR-1-era heading-first invalid evidence — keep reading exactly as persisted; they are never rewritten, revalidated or silently healed.
...
4. **FR-4 — Package validity and safety.** A candidate must have valid metadata and `SKILL.md` conforming to the THR-262 / seq27 authoring contract (a column-zero `---` frontmatter fence, a YAML mapping, a closing `---` fence; the body-heading requirement is retired and the body may be empty; the only permitted top-level keys are `name`, `description`, `license`, `compatibility` and `metadata`; the logical new-write slug must be a literal ASCII `^[a-z0-9]+(?:-[a-z0-9]+)*$` value of 1-64 characters and the frontmatter `name` must equal it; `description` must be a non-empty string of at most 1024 characters; optional fields are typed and duplicate top-level keys are rejected); cannot use protected/shipped/system slugs or namespace; cannot declare system contracts; and cannot include executable, credential, permission, sandbox, allow-rule, executor, or eligibility-writing behavior. The request-identity gate precedes validation and every durable write: a non-conforming logical slug (non-ASCII, lookalike, fullwidth, non-ASCII digit, uppercase ASCII, over-length, hyphen-boundary, or a synthetic non-conforming stored slug on append) is refused with HTTP 422 `invalid_slug` and zero residue, separately from a 201 immutable invalid-evidence document finding under an admitted ASCII identity. Validation completes before any persistence and the deterministic validator records its rule/validator version (`THR-262/1.0.0` for new rows) and findings by package hash, with structural findings evaluated before admission before required/optional field findings. An invalid candidate is appended as immutable validation/provenance evidence — one version row with deterministic findings, author/task/session provenance, and its content-addressed artifact — but never displaces an existing valid `current_version_id`, never projects a catalog description, never becomes eligible (`current_version_invalid`), and is never materialized. Only initial creation with no prior version sets the first (even invalid) version as the current pointer. A valid candidate projects its frontmatter `description` onto the catalog record in the same transaction that advances the pointer; an omitted or JSON-`null` request `description` derives from it, while an explicitly supplied API string (including `""` or whitespace-only) is compared and a differing value is rejected pre-persistence with HTTP 422 `divergent_description`. Protected-slug candidates are hard-rejected (a policy gate, not evidence); missing-body/missing-metadata requests are rejected with no residue. Append-only `(skill_id, content_hash)` uniqueness is never relaxed; stored pre-THR-262 records are never rewritten or silently healed. A byte-identical replay of any stored version (current or historical, valid or invalid evidence) conflicts as HTTP 409 `version_content_exists` before any divergent-description check, with zero residue — no artifact write or rewrite, no new version row, no new event/audit row, no `current_version_id` change; the 409 translation applies only to the version INSERT, while any later persistence failure stays a 500 with full artifact compensation. A `description` PATCH is evaluated against the current pointer re-read inside `BEGIN IMMEDIATE`: a valid current with a frontmatter description accepts only the equal value (else 422 `description_divergence`), while an invalid or legacy heading-first current accepts only the unchanged stored value (else 422 `description_requires_valid_version`); the display `name` stays independently editable.
