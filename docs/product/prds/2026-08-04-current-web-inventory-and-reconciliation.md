...
| Skills catalog/detail/status | Catalog/detail/status are read-only. Direct `POST /skills`, `PATCH /skills/{skill_id}`, direct validation, and direct assignment return `410 Gone` / `legacy_cutover`; `/skills/new` and `/skills/:skillId/edit` render Not Found. | Current contract: direct human authoring, proposal lifecycle, and eligibility remediation are retired. B1 verified-agent creation remains the only creation path, preserving its server-derived provenance, authorization boundary, and default-hidden status. |
| Retired Skills proposal surfaces | Proposal queue/detail UI, navigation, CLI, and routes are absent. | Historical proposal records do not define a current UI, API, or lifecycle commitment. |
...
- **Founder + Engineering gate:** any new Skills lifecycle, proposal participant/action, direct catalog write, human editor, eligibility-remediation, delegated role, or assistant governance write requires a separately authorized authority design. The retired surfaces are not a roadmap.
