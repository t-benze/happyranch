...
| Skills catalog/detail/create/edit/validation/assignment | catalog, validate, status, per-agent effectiveness | Skills PRD FR-1–9; source gates and guidance-only boundary. |
| Skills proposals queue/detail | founder-only claim/validate/submit-review/approve/reject; narrow verified-session pilot | Skills FR-10–13; publish/assign/rollback UI is explicitly excluded. |
...
Retained decisions: (1) Work Hours uses implemented leaf resolution, one org gate plus eligibility, atomic validation/last-known-good, and its wake-to-root-task contract; (2) skills preserve the immutable proposal/decision ledger, separate assignment/effectiveness projections, terminal rejection, and read-only source gates; (3) browser authority remains founder-authenticated; and (4) assistant conversations and tool evidence do not imply direct approval authority.
...
- **Founder + Engineering gate:** proposal participant/action expansion, publication/assignment/rollback controls, delegated roles, or assistant governance writes require authority design. The sole currently requested Founder decision is whether to authorize a bounded, auditable assistant governance-write scope; absent that ruling, the read-oriented posture remains.
