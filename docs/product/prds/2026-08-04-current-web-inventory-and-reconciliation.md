...
| Settings: assistant/org/executors; retired system/agents and unknown subroutes redirect to Assistant | org/team writes; runtime/executor/profile/adapter contracts | Core FR-19–22. `reviewer_agents` and `threads.default_turn_cap` remain operator-only with no browser controls. |
