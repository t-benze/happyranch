...
| S-5 | **Threads** — list + detail reshape (§4.2) | RENDER-ONLY | M | **@mention BROADCAST-ONLY** — no UI implying routing (A2/P1, dominant lens here). System/dispatch events visually distinct; **no synthesized in-transcript "ran:" cards** (D7 deferred). **Consumes the A4 marker.** |
| S-6 | **Tasks** — list + detail reshape (§4.3) | RENDER-ONLY | M | **List, not kanban.** **Roots-only by default on the parent→subtask axis** (one row per root; `task_type='subtask'`/`parent_task_id` not their own rows) with a **severity-max rollup** (worst descendant status lights up its root) + **subtasks always in the task detail** — **no in-list toggle in v1** (THR-010 msg 100; ADDITIVE, revisit/supersede chain UNCHANGED; derive over existing child statuses, **no schema**). Brief = raw monospace markdown, **no slot parser** (P1). Bidirectional lineage from real `revisit_of_task_id` / `walk_revisit_chain()`. Blocked node names its real `blocked_on`. |
