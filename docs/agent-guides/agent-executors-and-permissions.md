...
required by its own agentic CLI. It MUST NOT expect the daemon to translate a
generic permission setting or provider-specific allow-rule syntax into that
CLI. The stable v1 ``AdapterInput`` remains provider-neutral. Wrappers must
also preserve the daemon-provided callback environment, including ``PATH``, so
the agent can make required ``happyranch`` callbacks after ordinary workspace
actions. This is verified through wrapper review and founder approval; it does
not add daemon-managed permission enforcement. Founder approval must include
