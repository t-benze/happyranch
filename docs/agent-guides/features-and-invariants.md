...
- **Startup recovery (GH-688 Phase 1).** `_sweep_on_startup` replaces only the conversational `REPLY` portion of the generic reaper: valid queued wakes are retained and re-enqueued; an interrupted running `REPLY` is terminalized once as `daemon_restart` and replaced by exactly one queued wake. `BOOTSTRAP` and `TASK_FOLLOWUP` keep the generic `daemon_restart` reaping unchanged.
