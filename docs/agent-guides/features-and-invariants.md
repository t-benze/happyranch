...
- **Web UI.** React SPA with a grouped left sidebar nav (Primary: Home · Threads · Tasks · Agents · Knowledge · Artifacts; Operate: Spend · Dreams · Work Hours · Audit; footer: Settings + org switcher + theme toggle) + desktop window chrome, served from `web/dist/`. **Default landing route is Home** (the Dashboard page at `/orgs/:slug/dashboard`). Specs `docs/superpowers/specs/2026-05-14-web-ui-design.md`, `docs/superpowers/specs/2026-05-30-dashboard-overhaul-design.md`, `docs/design-overhaul/product_lead-2026-06-16-design-overhaul-PRD-build-spec.md` (Direction-A IA); architecture `web/ARCHITECTURE.md`; guide `docs/agent-guides/web-and-cli.md`.
...
- Agent replies no longer enforce a hard `turn_cap` ceiling; turn count/cap fields may still be tracked for compatibility/history but no longer gate posting/minting and are not surfaced as a turn budget UI per THR-046.
