...
**Thread routing is UNCONDITIONAL (founder ruling TASK-6027).** Mention routing and the strict mention-led exchange are default and permanent behavior — there is no switch, route, CLI verb, UI control, or config key for either. The shipped `threads.mention_routing_enabled` column (INTEGER NOT NULL DEFAULT 1) is an INERT legacy compatibility field: it must never be read to alter behavior, written through a product surface, or exposed on the wire (persisted true/false values cannot disable routing after upgrade); the proposed `reply_exchange_enabled` column and `org_settings.threads.reply_exchange_enabled` key were removed before shipping. **Mention routing (THR-198, Slice B) is the production routing contract:** the additive storage `thread_messages.mentions_json` (TEXT) persists a server-derived structured mention signal, and the two conversational store seams resolve the wake set at message-write time via `resolve_wake_set` (`runtime/daemon/thread_mentions.py`). Ratified matrix: **one/more valid current-participant mentions** → exactly that stable deduplicated set (speaker excluded); **zero valid mentions** — including no mentions, invalid/nonparticipant-only (`@founder`, typos, terminated), and self-only bodies — → full participant broadcast minus speaker. The signal is derived from `body_markdown` at write time (never client-declared; system/decline rows and pre-change history stay NULL). Routing is **write-time frozen**: a participant change after the write does not retroactively re-route already-minted wakes — the next message re-resolves against the current roster. **TASK_FOLLOWUP and BOOTSTRAP are isolated and NEVER mention-routed** (their replies keep the full broadcast). `addressed_to_json` remains unwritten/unread (its separate cleanup plan is intact). The Slice-C per-thread web control, the Slice-B founder toggle (`POST /threads/{id}/mention-routing`, audited `thread_mention_routing_changed`), and `happyranch threads mention-routing` were REMOVED by the TASK-6027 ruling — routing is unconditional; the `GET /threads*` wire no longer exposes either boolean. **Slice D (merged) ships the read-only release-measurement harness** (`runtime/infrastructure/thread_release_measurement.py`, documented in `docs/operations/gh-688-phase1-release-checklist.md` §8) reproducing the ratified acceptance metrics (G1 mentioned-message saving 293/499→~24/204, G2 founder coverage 698/0-loss, G3 org-wide decline report-only) over the Phase-2 epoch `2026-08-26T14:25:23Z` with interim labeling until the one-month window completes. **The strict mention-led exchange (TASK-5966) is unconditional** (founder-ratified THR-198 seq 194/195/196; `EXCHANGE_GRACE = 5 min`, absolute fail-open `MAX_PRIORITY_WAIT = 4 h`). A founder-authored `kind=message` with a non-empty valid mention set `P` and a non-empty deferred set `D = recipients − speaker − P` OPENS a per-thread exchange (`thread_reply_exchange`, at most one open per thread via the partial-unique index; `thread_exchange_deferrals` freezes D(E) at open). Inside the exchange: no-mention messages wake ONLY the frozen cohort `P(E)` (read from the immutable opening `mentions_json`); mention-pierce wakes the mentioned agent immediately WITHOUT joining the cohort; deferred pairs get obligation-only raises (U0 — `required` advances for EVERY recipient of every conversational message, separated from wake-mint eligibility; no token, no wake audit, watermarks stay monotonic/contiguous). Closure fires on (a) quiescence — no cohort member has a live wake whose coverage contains the exchange (running `running_through >= open_seq` or queued `required >= open_seq`; a pre-arrival claim never covers) — PLUS 5 minutes with no activity, (b) the 4-hour absolute reaper (fail-open), or (c) suppression (abort/archive/removal/corruption — no catch-up). On close/release, EVERY pair with `acknowledged < required` gets exactly ONE slot-checked catch-up wake covering the full contiguous range (the coverage-safe superset: frozen deferred set, mid-E joiners, cohort residual); a pair whose live slot GENUINELY covers the released range (running `running_through >= required`, or any queued wake — claim-time coverage is `acknowledged+1..required` and `required` is monotonic) is coalesced (never a second token); a released deferral whose live slot does NOT cover it (a pre-arrival claim with immutable `running_through < required`) is durably marked `catchup_pending` and receives exactly ONE post-slot catch-up when the blocking wake reaches a TERMINAL outcome — failed/timeout settlement, or a recoverable restart terminalization (survives daemon restart; never stranded, TASK-6057). Fail-closed recovery classifications (both-slots corruption, non-recoverable/invalid/malformed running or queued ownership) REVOKE the pending marker atomically in the same transaction and never mint a catch-up from state recovery declared untrustworthy (TASK-6065); the retained unacknowledged range is then served by the next conversational arrival (Phase-1 fail-open). `daemon_restart` is an interruption, never terminal, while a recovery replacement covering the exchange is queued/running (reconcile step 6e keeps the cohort non-quiescent). Corruption sweeps fail closed (suppress + diagnostic). **Rollback is code-version rollback / operational containment only** (TASK-6027 — the per-thread toggle, the org kill-switch, and the `exchange_disabled`/`org_exchange_disabled` close reasons were removed; the F1 measurement boundary `RATIFIED_EPOCH_F1` is PRESERVED; Phase-2 epoch/window untouched; no backfill). Reverting the code stops new exchanges; an open epoch from a reverted deploy is closed by the normal idempotent closure bounds (quiescence + 5-min grace or the 4h fail-open reaper; startup reconcile 6e and the write seams evaluate the same bounds) — never stranded. Any-burst deferral and the substantive-peer-engagement doctrine amendment remain separately gated. **Observation remains pending — a shipped diagnostic is not proof of the population outcome.**
...
- **Coalescing (GH-688 Phase 1).** A burst creates at most one unstarted `REPLY` per pair; new queue tokens are enqueued only after the arrival transaction commits. An arrival while queued or running only advances `required_through_seq`. A successful reply/decline acknowledges **only the claimed coverage** (the immutable `running_through_seq` snapshot at claim); arrivals during the run yield exactly one post-settlement follow-on. Failure/timeout never hot-loop: the range stays unacknowledged, projects `retry_required`, and the next conversational arrival covers the retained plus new range. THR-200 PR E adds an orthogonal durable provider breaker: exactly three consecutive structured post-launch failures OPEN it; OPEN launches nothing, and a timer admits one unique half-open probe at 15 minutes unless the pair is held in an open exchange. Gap/full-transcript recovery never counts and never depends on an episode row.
- **Breaker projection (THR-200 PR E).** The pair wire enum adds `breaker_open` and `probe`. Held open-exchange deferral remains neutral and excludes probe admission; otherwise active breaker state supersedes ordinary queue/retry diagnostics. Exact visible copy is “Reply paused after repeated provider failures · messages F–T”, “Recovery will be tested after <time>.”, and “Testing provider recovery · messages F–T.” These states never fabricate typing, retrying, or recovered evidence.
...
- **Wire contract (GH-688 Phase 1).** `GET /threads/{id}` and `GET /threads/{id}/messages` carry a pair-level `reply_delivery` projection (running | queued | held | retry_required, inclusive `from_seq`/`through_seq`, store-computed `coalesced_message_count`, `started_at`, `last_terminal_reason`). Precedence is running > queued > held > retry_required > settled omission. Held requires BOTH an OPEN `thread_reply_exchange` and the participant's matching HELD `thread_exchange_deferrals` row; released/suppressed/closed/missing-half history never masks a genuine retry. Terminal reason is historical metadata, exposed only on a genuine current retry and never classification proof or fault copy for held. The projection is never fabricated from per-message rows and never claims a subprocess exists beyond `started_at`. Historical per-message `responder_status` strips are unchanged in shape and now ALSO carry the authoritative invocation `purpose` (`reply` | `task_followup`; BOOTSTRAP stays excluded from the grouped query) so wire classification never has to infer purpose from the triggering row kind — a coalesced REPLY delivery range can anchor on a SYSTEM row (the follow-on mint keys the first unacknowledged sequence, which may be a system divider).
- **UI presentation (GH-688 Phase 1 Slice C).** The thread detail rail shows a compact "Reply delivery" section driven ONLY by the store projection: current `running` pairs stay individually visible first with full wrapping agent identities; `queued` pairs are visually distinct and grouped behind a native keyboard/screen-reader disclosure with truthful singular/plural counts; `held` stays visible as healthy neutral "waiting for current exchange" and is never typing, an active subprocess, or a fault; `retry_required` stays visible as an amber diagnostic. Detail captions preserve the inclusive range: queued reads "N message(s) coalesced · message(s) F–T" (static — never an active subprocess), running reads "replying · messages F–T" (the only subprocess evidence is `started_at`), held reads "waiting for current exchange · messages F–T", and retry-required reads "retry required · messages F–T · last: <reason>" only when the current retry has reliable terminal metadata. Fully settled pairs remain absent from this live store projection; their truthful terminal evidence remains in the per-message responder history rather than being recreated in the rail. The transcript tail's live indicator (TypingBubble) is driven only by queued/running pair projection; held and retry_required remain rail-only. Inferred per-message rows remain only for special-purpose wakes (BOOTSTRAP / TASK_FOLLOWUP) that are intentionally outside `reply_delivery` — preserved even when the same agent concurrently holds a conversational REPLY pair. Inferred-row suppression is purpose-aware (the wire `purpose`, never the triggering row's kind): a REPLY whose coalesced range anchors on a system row still reads `purpose=reply` and is masked by its pair row — exactly one replying bubble — while a same-agent TASK_FOLLOWUP stays visible (never agent-name-only). Per-message terminal responder history is unchanged (and now also renders under SYSTEM rows, so a settled system-row-anchored REPLY range shows its terminal `replied` marker). SSE tail events and the send/invite/remove/archive/resume/abort mutations invalidate the thread-detail query so the projection stays fresh — no new wire events were added.
...
- **Strict exchange (TASK-5966).** The exchange-epoch gate suppresses every deferred-pair wake mint for the duration of the exchange (no-pierce property — the only pierce sources are a mention of the agent, a pre-existing queued wake claimed mid-exchange, and TASK_FOLLOWUP/BOOTSTRAP isolated broadcasts) and mints exactly ONE slot-checked catch-up per pair at closure covering `acknowledged+1..required`. The settled audit payload is extended (`covered_from_seq/covered_through_seq/claimed_at/minted_at/exchange_held`) so tightened D1 attribution is durable: a wake claimed before a message (`claimed_at < M.created_at`, `running_through < M.seq`) can never be its covering substantive reply; a queued settlement attributes only messages minted-at-or-before the wake. Closure is an atomic CAS (`open → released|suppressed`); duplicate evaluations are silent misses (no audit, no double catch-up). Abort/archive suppress the exchange with no catch-up; participant removal suppresses the removed member's deferral rows and the removed pair never receives a catch-up. The reaper (30s tick, `runtime/daemon/exchange_reaper.py`) enforces the 4h fail-open; startup reconcile (step 6e) runs the corruption sweep (5 fail-closed classes) then idempotent closure evaluation. The exchange is UNCONDITIONAL (TASK-6027): there is no disable path — an open epoch is closed only by the ratified bounds (quiescence + grace, 4h fail-open, or suppression), and code-version rollback is the only operational containment.
