...
def migrate_to_multi_org(
    ...
    *,
    ...
    if not slug:
        raise RuntimeError(f"v1 marker at {rt}/happyranch.yaml is missing 'slug'")

    db_path = rt / "happyranch.db"
    ...
    plan = {
        ...
        "would_move": [
            ...
            (str(rt / "happyranch.db"), str(new_org_root / "happyranch.db")),
        ],
    ...
    if db_path.exists():
        shutil.move(str(db_path), str(new_org_root / "happyranch.db"))
    ...
    (rt / "happyranch.yaml").write_text(yaml.safe_dump(new_marker, sort_keys=False))
