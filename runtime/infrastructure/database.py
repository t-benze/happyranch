...
class Database:
    ...
    def set_thread_exchange_routing_with_audit(
        ...
        audit row (TASK-5966 independent rollback control).

        ONE rollback-safe transaction: the authoritative
        ``reply_exchange_enabled`` read, the idempotence decision, the state
        UPDATE, and the ``thread_exchange_routing_changed`` audit row commit
        together — and roll back together on ANY failure. Same-state requests
        are true no-ops (no write, no audit). ``mention_routing_enabled`` is
        NEVER touched — this is the separate mode-2 switch.

        Returns True when a durable transition occurred; False for a
        same-state (no-op) save. Raises ValueError for an unknown thread.
        """
        try:
            self._conn.execute("BEGIN IMMEDIATE")
            row = self._conn.execute(
