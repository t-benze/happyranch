...
def purge(slug: str, skill_id: str, body: dict = Body(...), org: OrgDep = None, _: None = Depends(_require_human)):
    ...
    Lock order is publication fence -> DB lock/BEGIN IMMEDIATE.  The commit is
    the linearization point; no filesystem object or retained evidence is
    removed.  An identical or later retry returns the stable tombstone.
    """
    from runtime.skills.custom.fence import custom_skill_publication_fence

    conn = getattr(org.db, "_conn", org.db)
    _purge_contract(conn)
    with custom_skill_publication_fence(slug, write=True):
        conn.execute("BEGIN IMMEDIATE")
        try:
            row = service.current(conn, skill_id)
            ...
            # This also keeps the preserved old resolver fail-closed if an
            # operator deliberately downgrades and restores this exact row.
