...
# Daemon-state path locators. These live here (not in runtime.daemon.paths) so
# the top-level cli/ package can import them without reaching into
# runtime.daemon. runtime/daemon/paths.py re-exports them so existing daemon
# call sites stay unchanged. They depend only on os/pathlib — no daemon import,
# so importing runtime.runtime never pulls in the daemon package.
