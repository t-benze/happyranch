...
- **Host-resource concurrency (THR-207 / TASK-5584 Slice A):** load-bearing ordering invariants — no agent subprocess launches before a daemon-wide admission lease; every terminal path (success/nonzero/timeout/cancel/retry) finishes containment and reconciles residue before lease release; queued cancellation never launches; cancellation goes through the opaque containment handle, not a bare PID signal. Lifecycle is ONE atomic ownership protocol: ownership transfers at admission grant, the durable first-wins terminal reason lives on the ownership record, and the daemon drain iterates the same registry — a shutdown at/after grant is never lost. Contracts live in `runtime/orchestrator/host_supervisor.py` + `runtime/platform/session_backend.py`; governing spec `docs/superpowers/specs/2026-08-24-host-resource-concurrency.md`. Per the founder-approved real-caller amendment (THR-207 seq 41–44) Slice A wires exactly ONE producer — schedule fires (`runtime/daemon/schedule_runner.py`) through the honest no-enforcement `PassthroughBackend` — and the app-lifespan drain calls `supervisor.shutdown()`; `runtime/platform/isolation.py` is untouched, and task/thread/dream/wake producers stay structurally unchanged (later slices wire them against the same contract).
