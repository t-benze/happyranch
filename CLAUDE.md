...
- **Phase-2 mention routing (THR-198) — Slice A storage is merged but routing is NOT enabled.** Slice A (TASK-5660) lands additive, idempotent columns `threads.mention_routing_enabled INTEGER NOT NULL DEFAULT 1` (per-thread switch, default enabled for existing + new threads) and `thread_messages.mentions_json TEXT` (structured mention signal), plus the pure parse/resolver module `runtime/daemon/thread_mentions.py`. The two conversational store seams (`record_conversational_arrival`, `reply_conversational`) derive and persist `mentions_json` from `body_markdown` at write time (canonical valid participant mentions, speaker excluded; `[]` for zero-valid; NULL for system/decline rows and pre-change history). **Production wake routing is intentionally UNCHANGED until Slice B** — every message still broadcasts to participants minus speaker; do not rely on `mentions_json` for routing and do not revive the dead `thread_messages.addressed_to_json` (its separate cleanup plan is intact).
