...
- **Metrics persistence** (THR-066 + TASK-5443): daemon-global `metrics.db` at `<runtime_root>/metrics.db` (NOT per-org). Append-only snapshots every ~60s, 30-day retention. The store is `runtime/daemon/metrics_store.py`; the periodic writer piggybacks `work_hours_scheduler_loop`. **Routes**: `GET /api/v1/metrics` (live snapshot + pull-gauges), `GET /api/v1/metrics/history` (persisted rows, newest-first, bearer-authed, `?since=&until=&limit=` query params), `POST /api/v1/metrics/maintenance` (bearer-authed, founder-explicit maintenance). HTTP latency is labelled by the matched FastAPI **route template** (e.g. `POST /api/v1/orgs/{slug}/tasks/{task_id}/completion`), never the literal `request.url.path`; an unmatched request records `METHOD __unmatched__` and a handler exception records `METHOD __error__` (elapsed time still recorded, exception re-raised). Snapshots carry an additive `format_version` marker (`2` = template labels; a stored row without it is legacy raw-path format and stays readable). Each persist cycle logs non-sensitive storage telemetry (route-label cardinality, serialized bytes, row/prune counts, oldest/newest capture, DB/WAL bytes, page/free-list counts). **Maintenance** (`POST /api/v1/metrics/maintenance` + `happyranch metrics maintenance --confirm-quiescent`): requires explicit confirmation — ONLY the JSON literal boolean `true` is accepted (`StrictBool`; coercible values like `"yes"`/`1`/`0` are rejected HTTP 422 before any admission or maintenance work) — then atomically enters an admission/drain/exclusivity gate — new normal traffic is rejected (HTTP 503), already-admitted requests drain (bounded), quiescence (no nonterminal task / running job / active executor session) is re-checked, and only then — through `MetricsStore` — does it prune at the unchanged 30-day cutoff, WAL checkpoint (fail-closed on `busy`), run `PRAGMA integrity_check` (fail-closed on non-`ok`), and `VACUUM`. A second concurrent call is deterministically rejected; the gate is released on every success/failure path; a failure returns HTTP 500 (`maintenance_failed` with a stable bounded `code`/`reason` — the original exception is logged server-side only and never echoed to the client) with history still queryable and no automatic retry. For the entire pending/active interval BOTH background producers — due schedule fires (`schedule_due_schedules`) and due working-hours wakes (`schedule_due_wakes`) — are admitted through a single gate-owned counted **background-work lease** (`MaintenanceGate.background_lease`) that is atomic with the OPEN→PENDING transition: a producer admitted while OPEN holds the lease across its ENTIRE authoritative path (schedule recovery/claim + audit + queue enqueue; work-hours insert + audit + queue enqueue), and a producer denied because maintenance won first defers with zero side effects. The periodic snapshot writer (`maybe_persist_metrics_snapshot`) uses the same lease, so a snapshot write can never begin after maintenance is PENDING or overlap checkpoint/VACUUM. Drain waits for admitted HTTP requests AND background leases before the route rechecks quiescence or calls `mark_active`, so ACTIVE cannot be entered while any producer pass or persist is mid-flight. Nothing is dropped or consumed: due schedules stay `ARMED` / due wake slots stay unscheduled and the first tick after the gate releases processes them unchanged while both scheduler loops stay alive; the loops' startup catch-up flag only advances once a pass actually runs. Never delete `metrics.db`, `-wal`, or `-shm` manually, and never run maintenance against a live runtime.
