...
- **Metrics persistence** (THR-066 + TASK-5443 remediation): daemon-global `metrics.db` at `<runtime_root>/metrics.db` (NOT per-org). Append-only snapshots every ~60s, 30-day retention (unchanged). The store is `runtime/daemon/metrics_store.py`; the periodic writer piggybacks `work_hours_scheduler_loop`. **HTTP labels are route TEMPLATES** (e.g. `POST /api/v1/orgs/{slug}/tasks/{task_id}/completion`) — never literal `request.url.path` — so dynamic IDs coalesce to bounded cardinality; unmatched/error requests use stable `METHOD __unmatched__` / `METHOD __error__` labels. Snapshots carry an additive `format_version` (`2` = template labels); a missing field means legacy raw-path labels and stays readable (rows are never rewritten). **Routes**: `GET /api/v1/metrics` (live snapshot + pull-gauges), `GET /api/v1/metrics/history` (persisted rows, newest-first, bearer-authed, `?since=&until=&limit=` query params), `POST /api/v1/metrics/maintenance` + `happyranch metrics maintenance --confirm-quiescent` (explicit quiescent prune + WAL checkpoint + `PRAGMA integrity_check` + `VACUUM`; refuses absent confirmation or non-quiescence). Never hand-delete `metrics.db`/`-wal`/`-shm`; maintenance runs only through the store.
