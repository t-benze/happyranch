...
- **Recovery (two distinct procedures, no single one-step command):** (a) Broken/missing workspace LINKS: ``happyranch set-executor <agent> --executor <current-executor>`` re-materializes symlinks from canonical store (links ONLY — does NOT recover corrupted canonical bytes). (b) Corrupted canonical BYTES (hash mismatch, tampered content): stop daemon, delete corrupted package directory under ``<daemon-home>/canonical-skills/<slug>/<version>/<hash>``, restart daemon so next materialization rebuilds from authoritative release/custom artifact source. No automatic repair from same-UID local sources.
